...
object TimeUsage extends TimeUsageInterface {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryNeeds = columnNames.filter(colName => List("t01", "t03", "t11", "t1801", "t1803").exists(colName.startsWith)).map(col(_))
    val work = columnNames.filter(colName => List("t05", "t1805").exists(colName.startsWith)).map(col(_))
    val other = columnNames.filter(colName => List("t02", "t04", "t06", "t07", "t08", "t09",
                                                   "t10", "t12", "t13", "t14", "t15", "t16", "t18").exists(colName.startsWith)
                                              && !List("t1801", "t1803", "t1805").exists(colName.startsWith)).map(col(_))
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    summed.groupBy($"working", $"sex", $"age")
      .agg(
        round(avg($"primaryneeds").alias("primaryneeds"), 1),
        round(avg($"work").alias("work"), 1),
        round(avg($"other").alias("other"), 1),
      )
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    def round(num: Double) = (num * 10.0).round / 10.0
    summed
